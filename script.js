const translations = {
  en: {
    ...
    expertise1Title: 'Connecting systems', expertise1Text: '<strong>Before:</strong> people copy the same data between applications or work around systems that do not communicate. <strong>After:</strong> the applications exchange the right data automatically through APIs, databases, or files.',
    ...
    expertise2Title: 'Keeping systems running', expertise2Text: '<strong>Before:</strong> when something fails, it is unclear whether the cause is code, a container, a database, or the network. <strong>After:</strong> the system is monitored, backed up, and easier to diagnose and restore.',
    expertise2Item2: 'Monitoring & troubleshooting', expertise2Item3: 'Deployments, backups & security',
    expertise3Title: 'Improving legacy software', expertise3Text: '<strong>Before:</strong> manual steps and old code make every change slow and risky. <strong>After:</strong> repeatable automation and tests make work faster and changes safer to release.',
    ...
    modernizationText: '<strong>Before:</strong> important business rules were spread across old PHP code, database rules, and scheduled jobs. Even a small change could break another process. <strong>After:</strong> I mapped the risky parts, removed obsolete files, and added automated tests, repeatable database changes, Docker services, and two controlled deployment environments. Changes can now be checked before release instead of relying only on manual testing.',
    ...
    case2Text: '<strong>Before:</strong> the legacy PHP system could not communicate directly with Poland’s National e-Invoice System (KSeF), which requires newer authentication and invoice formats. <strong>After:</strong> I added a separate Python service that converts invoice data to FA(3) XML, authenticates with KSeF, sends invoices, checks their status, and finds an invoice submitted earlier. The old system can now use KSeF without a risky rewrite of the whole application.',
    ...
    case4Kicker: 'Client loyalty product', case4Title: 'Personalized loyalty cards for Marta Banaszek Atelier Café',
    case4Text: '<strong>Before:</strong> customer and loyalty data stayed in the Dotykačka point-of-sale system, with no personalized card customers could keep on their phones. <strong>After:</strong> I connected Dotykačka to a Django application that creates a personal Marta Banaszek Atelier Café loyalty card for Apple Wallet or Google Wallet and gives staff one place to manage the program.',
    ...
    warsawMossAwardTitle: 'Eco Navigate — 3rd place', warsawMossAwardText: '<strong>Before:</strong> route planners mainly compared time and distance. <strong>After:</strong> Eco Navigate compares walking and cycling routes through Warsaw using data about nearby trees, parks, and other green areas. It recommends a greener option while showing its distance, time, and extra detour, and it also displays live air-quality stations.',
  ...
  pl: {
    ...
    expertise1Title: 'Łączenie systemów', expertise1Text: '<strong>Przed:</strong> ludzie kopiują te same dane między aplikacjami albo obchodzą systemy, które ze sobą nie rozmawiają. <strong>Po:</strong> aplikacje automatycznie wymieniają właściwe dane przez API, bazy danych lub pliki.',
    ...
    expertise2Title: 'Utrzymanie działających systemów', expertise2Text: '<strong>Przed:</strong> gdy coś przestaje działać, nie wiadomo, czy przyczyną jest kod, kontener, baza danych czy sieć. <strong>Po:</strong> system jest monitorowany, ma kopie zapasowe i łatwiej go zdiagnozować oraz przywrócić.',
    expertise2Item2: 'Monitoring i diagnozowanie problemów', expertise2Item3: 'Wdrożenia, kopie zapasowe i bezpieczeństwo',
    expertise3Title: 'Ulepszanie oprogramowania legacy', expertise3Text: '<strong>Przed:</strong> ręczne kroki i stary kod sprawiają, że każda zmiana jest powolna i ryzykowna. <strong>Po:</strong> powtarzalna automatyzacja i testy przyspieszają pracę oraz pozwalają bezpieczniej wdrażać zmiany.',
    ...
    modernizationText: '<strong>Przed:</strong> ważne reguły biznesowe były rozproszone między starym kodem PHP, bazą danych i zadaniami cyklicznymi. Nawet niewielka zmiana mogła uszkodzić inny proces. <strong>Po:</strong> zmapowałem ryzykowne miejsca, usunąłem niepotrzebne pliki oraz dodałem automatyczne testy, powtarzalne zmiany bazy danych, serwisy Docker i dwa kontrolowane środowiska wdrożeniowe. Zmiany można teraz sprawdzić przed wydaniem, zamiast polegać wyłącznie na testach ręcznych.',
    ...
    case1Text: '<strong>Przed:</strong> każda firma przesyłała pliki CSV, XLS lub XLSX z innymi kolumnami i układem. Nowy układ mógł zatrzymać import i wymagać zmiany kodu przez programistę. <strong>Po:</strong> zbudowałem jeden proces, który mapuje każdy plik do wspólnej struktury, pozwala operatorowi poprawić mapowanie i zapamiętuje udane układy na kolejne importy. Błędny wiersz trafia do logu zamiast zatrzymywać cały plik, a zadanie można zatrzymać i wznowić. Gdy tożsamość firmy jest niepewna, AI może zaproponować najbardziej prawdopodobne dopasowanie, ale człowiek musi je zatwierdzić przed zapisaniem wrażliwych danych.',
    ...
    case2Text: '<strong>Przed:</strong> system legacy w PHP nie potrafił komunikować się bezpośrednio z Krajowym Systemem e-Faktur (KSeF), który wymaga nowszego uwierzytelniania i formatu faktur. <strong>Po:</strong> dodałem osobny serwis w Pythonie, który przekształca dane faktury do XML FA(3), uwierzytelnia się w KSeF, wysyła dokument, sprawdza jego status i znajduje fakturę wysłaną wcześniej. Stary system może korzystać z KSeF bez ryzykownego przepisywania całej aplikacji.',
    ...
    case4Kicker: 'Produkt lojalnościowy dla klienta', case4Title: 'Spersonalizowane karty dla klientów Marta Banaszek Atelier Café',
    case4Text: '<strong>Przed:</strong> dane klientów i programu lojalnościowego pozostawały w systemie sprzedażowym Dotykačka, bez spersonalizowanej karty w telefonie klienta. <strong>Po:</strong> połączyłem Dotykačka z aplikacją Django, która tworzy osobistą kartę lojalnościową Marta Banaszek Atelier Café do Apple Wallet lub Google Wallet i daje obsłudze jedno miejsce do zarządzania programem.',
    ...
    warsawMossAwardTitle: 'Eco Navigate — 3. miejsce', warsawMossAwardText: '<strong>Przed:</strong> nawigacje porównywały głównie czas i odległość. <strong>Po:</strong> Eco Navigate porównuje trasy piesze i rowerowe przez Warszawę na podstawie danych o pobliskich drzewach, parkach i innych terenach zielonych. Poleca bardziej zieloną opcję, pokazując jej długość, czas i dodatkowe nadłożenie drogi, a także wyświetla aktualne stacje jakości powietrza.',
